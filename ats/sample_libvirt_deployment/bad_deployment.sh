...
litp create -t eth -p /deployments/single_blade/clusters/cluster1/nodes/node1/network_interfaces/if0 -o device_name=eth0 macaddress='DE:AD:BE:EF:45:50' bridge=br0
litp create -t bridge -p /deployments/single_blade/clusters/cluster1/nodes/node1/network_interfaces/br0 -o device_name=br0 network_name=nodes ipaddress='10.10.10.103' forwarding_delay=4 stp=false
